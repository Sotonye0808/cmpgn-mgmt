...
- CIS uses a push model: CIS posts signed webhooks to `/api/cis/webhook` on this platform.
- The webhook stores identity mappings in `CisIdentity` and event history in `CisWebhookEvent` without mutating local user records.
- `GET /api/cis/status` exposes readiness and webhook configuration for operators.

...
export default function SmartLinkRedirect({ slug, destination }: Props) {
  useEffect(() => {
    // Persist the referral slug so registration can attribute it even after
    // multiple page navigations (e.g. campaign page → register)
    try {
      sessionStorage.setItem("ref_slug", slug);
    } catch {
      ...
      <div className="flex flex-col items-center gap-4 text-center px-6">
        ...
        <noscript>
          <p className="text-ds-text-secondary text-sm">
            ...
            <a href={destination} className="text-ds-brand-accent underline">
